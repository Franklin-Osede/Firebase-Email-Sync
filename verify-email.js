...
// Email que queremos verificar (ya actualizado)
const emailToVerify = 'dominguezrodrigomm@gmail.com';

// Busca el usuario con el nuevo email
admin.auth().getUserByEmail(emailToVerify)
  .then((userRecord) => {
    console.log('Usuario encontrado:', userRecord.uid);

    // Solo actualiza el campo emailVerified a true
    return admin.auth().updateUser(userRecord.uid, {
      emailVerified: true
    });
  })
  .then((userRecord) => {
    console.log('Email verificado correctamente:', userRecord.toJSON());
    process.exit(0);
  })
  .catch((error) => {
